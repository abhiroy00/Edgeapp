import React from 'react'

export default function Asset_Information() {
  return (
    <div>
        <div
        className="w-full mt-30 shadow-2xl rounded-2xl p-8 mb-8"
        style={{ backgroundColor: "#d8eefc" }}
      >
        <div className="flex flex-col md:flex-row md:justify-between gap-4">
          <p className="font-bold text-lg">
            Station : Pt Ram Prasad Bismil (PRPM)
          </p>
          <p className="font-bold text-lg">Division : Moradabad</p>
          <p className="font-bold text-lg">Zone : Northern Railway</p>
        </div>
      </div>

      <ul className='flex gap-20 mt-10'>
            <li className='font-bold text-2xl'>• Asset Name</li>
            <li className='font-bold text-2xl'>Point Machine </li>
            <li className='font-bold text-2xl'>Parameters monitored</li>
        </ul>

        <ul className='flex gap-20 mt-10'>
            <li className='font-bold text-2xl'>• Asset Name</li>
            <li className='font-bold text-2xl'>Point Machine </li>
            <li className='font-bold text-2xl'>Parameters monitored</li>
        </ul>
    </div>
  )
}
